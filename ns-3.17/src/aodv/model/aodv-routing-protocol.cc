...
RoutingProtocol::RoutingProtocol () :
  ...
  mc_RouteTimer (Timer::CANCEL_ON_DESTROY)
{
  if (EnableHello)
    {
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  this->isSU = m_ipv4->GetAddress (i, 0).GetLocal().isSU();
  if(isSU)NS_LOG_FUNCTION (m_ipv4->GetAddress (i, 0).GetLocal () << "is a primary user");
  else NS_LOG_FUNCTION (m_ipv4->GetAddress (i, 0).GetLocal () << "is a secondary user");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  if (isSU) {
    ...
    {
      //
      //mc_RouteTimer.SetFunction()
      
      mc_RouteTimer.SetFunction (&RoutingProtocol::MC_RouteTimerExpire, this);
      mc_RouteTimer.Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 100)));
    } else {
      //
      return;
    }
...
void
RoutingProtocol::MC_RouteTimerExpire ()
{
  NS_LOG_FUNCTION (this);
  //SendHello ();
  mc_RouteTimer.Cancel ();
